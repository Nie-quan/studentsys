...
def show_student(lst):
    ...
    for item in lst:
        sum = item.get('english') + item.get('python') + item.get('java')
        ...
        if student_id != '':
            if os.path.exists(filename):
                with open(filename, 'r', encoding='utf-8') as file:
                    student_old = file.readlines()  # 返回值是列表
            else:
                student_old = []
            flag = False  # 标记是否删除
            if student_old:
                with open(filename, 'w', encoding='utf-8') as wfile:
                    ...
                    for item in student_old:
                        ...
                        if d['id'] != student_id:
                            ...
                            flag = True
                    if flag:
                        print('id为{0}的学生信息被删除'.format(student_id))
                    else:
...
def sort():
    ...
    if asc_or_desc == '0':
        ...
        print('您的输入有误，请重新输入！！！')
        sort()
    mode = input('请选择排序方式（1.按英语成绩 2.按python成绩 3.按java成绩 0.按总成绩成绩）：')
